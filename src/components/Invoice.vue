      ...
      <b-col cols="12">
        <b-button-group class="w-100">
          <b-button @click="add" variant="success">
            Add Entry <b-icon-plus-square></b-icon-plus-square>
          </b-button>
          <b-button @click="download" variant="info" v-if="data.entries.length > 0">
            Download Invoice <b-icon-download></b-icon-download>
          </b-button>
          <b-button @click="exportJson" variant="warning" v-if="data.entries.length > 0">
            Export Data <b-icon-file-code></b-icon-file-code>
          </b-button>
        </b-button-group>
        <hr/>
            ...
            <b-row>
              ...
              <b-col cols="12" md="4" class="mt-2">
                <b-form-group label="Hours">
                  <b-input type="number" step="0.5" min="0" v-model="entry.time" :disabled="entry.disabled"></b-input>
                </b-form-group>
              </b-col>
              <b-col cols="12" md="4">
...
export default {
  ...
  methods: {
    ...
    edit(index) {
      this.data.entries[index].disabled = false
    },
    disable(index) {
      this.data.entries[index].disabled = true
    },
    del(index) {
